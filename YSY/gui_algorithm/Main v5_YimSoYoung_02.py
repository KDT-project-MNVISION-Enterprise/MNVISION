...
# 변주영 ===================================================================
test_filepath = r"C:\Users\kdp\Desktop\test_video_02.mp4"
omp3_file = r"Program\Audio\alarm_bell.mp3"
form_class = uic.loadUiType(r"Program/UI/Video.ui")[0]
mp3_file = "Program/Audio/alarm_bell.mp3"

m_file = r"C:\Users\kdp\Desktop\Model_v5.pt"
five_repo = r"C:\Users\kdp\Desktop\yolov5"
ort_session = torch.hub.load(five_repo, 'custom', path=m_file, source='local') # ⭐
rt_session2 = torch.hub.load(five_repo, 'custom', path=m_file, source='local') # ⭐
# =========================================================================
...
class ObjectDetection:
    ...
    def yimsoyoung(self, list_ysy, class_ids, label, list_box, value, value2, cv2_list):
            ...
            if label == 'Person' : 
                ...
                if forklift_box : 
                    ...
                    if (f_x1-50 <= x2 <= f_x2+50) and (f_y1-50 <= y2 <= f_y2+50):
                        cv2_list.append(('Person on FORKLIFT', (600,350), self.font, 1, self.b_c, 3))
        ...
        # Forklift가 Rack 공간에 작업 중인 경우 알림 표시 
            ...
            if (value < 500) or (value2 < 500):
                if value < value2:
                    cv2_list.append(('Folklift on UPPER RACK', (300,400), self.font, 1, self.y_c, 3))
                else:
                    cv2_list.append(('Folklift on LOWER RACK', (300,400), self.font, 1, self.y_c, 3))
        else:
            self.count = 1
    ...
    def detect_danger(self, results, forklift_frames, forklift_valid, forklift_moves):
        ...
        if person_valid and self.detect_danger_between_forklift_and_person(forklift_frames, person_frame):
            ...
            # print('[4] 실행')
            self.cv2_labels.append(('collision risk occurred :o', (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)) # 😎
            self.cv2_labels.append((f'{person_frame}', (30, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 3)) # 테스트용
        ...
        if forklift_valid and forklift_moves: # 😎
            ...
            if forklift_moves:
                ...
                self.cv2_labels.append((point1, point2, (0, 0, 255), 5)) # 😎
                dist = self.euclidean_dist(x1, y1, x2, y2)   # 빼도 되나?
                self.cv2_labels.append((f'Dist : {dist:.3f}', (1030, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)) # 😎
    ...
    def apply_model(self, frame, upper_coordinates=None, lower_coordinates=None):
        ...
        if upper_coordinates is None and lower_coordinates is None:
            ...
            for *box, conf, cls in results_data:
        ...
        if len(self.cv2_labels) > 0 :
            for k in range(0, len(self.cv2_labels)):
                if isinstance(self.cv2_labels[k][0], str):
                    # 프레임, 텍스트 내용, 넣을 위치, 폰트 종류, 폰트 크기, 폰트 색, 폰트 굵기
                    cv2.putText(annotated_frame, self.cv2_labels[k][0], self.cv2_labels[k][1], self.cv2_labels[k][2], self.cv2_labels[k][3], self.cv2_labels[k][4], self.cv2_labels[k][5])
                else:
                    # 프레임, 좌표1, 좌표2, 색상, 굵기
                    cv2.line(annotated_frame, self.cv2_labels[k][0], self.cv2_labels[k][1], self.cv2_labels[k][2], self.cv2_labels[k][3])
